...
public class Requetes {
	...
	public Requetes() {
		this.cs = ConnectionSingleton.getInstance();
		this.c = cs.getConnection();
	}
	...
	public List<Article> requete_1(String nom) {
		...
		try {
			PreparedStatement ps = c.prepareStatement("select article.titre from article "
	...
	public List<String> requete_3() {
		...
		try {
			PreparedStatement ps = c.prepareStatement("select email, nomlabo from travailler "
					+ "group by email "
					+ "order by email asc, nomlabo asc");
	...
	public List<Chercheur> requete_4(int nb) {
		...
		try {
			PreparedStatement ps = c.prepareStatement("select email, count(titre) from annoter "
					+ "group by email "
					+ "having count(titre) >= ? "
					+ "order by email");
	...
	public double requete_5(String email) {
		...
		try {
			PreparedStatement ps = c.prepareStatement("select email, avg(note) from noter "
					+ "where email = ?");
	...
	public List<String> requete_6(String labo) {
		...
		try {
			PreparedStatement ps = c.prepareStatement("select ecrire.email, count(ecrire.titre), count(note), avg(note) from ecrire "
					+ "inner join noter on ecrire.titre = noter.titre "
					+ "inner join travailler on ecrire.email = travailler.email "
					+ "where nomlabo = ? "
					+ "group by ecrire.email "
					+ "order by avg(note) desc, ecrire.email asc");
			...
			if (!res.wasNull()) {
				while (res.next()) {
					Chercheur cherch = new Chercheur(res.getString("email"));
					list.add(cherch.toString() + " | Article(s) ecrit: " + res.getInt("count(titre)") + " | Note(s) obtenue(s): " + res.getInt("count(note)") + " | Moyenne: " + res.getDouble("avg(note)"));
				}
			}
	...
	public boolean requete_7(String article) {
		...
		try {
			PreparedStatement ps_1 = c.prepareStatement("select travailler.nomlabo from travailler "
					+ "inner join ecrire on travailler.email = ecrire.email "
					+ "where titre = ?");
			...
			PreparedStatement ps_2 = c.prepareStatement("select travailler.nomlabo from travailler "
					+ "inner join noter on travailler.email = noter.email "
					+ "where titre = ? and note = max(note)");
